...
int main(void) {
    ...
    address.sin_port = 0;
    CHECK(bind(server.listener, (struct sockaddr *)&address,
               sizeof(address)) == 0);
    CHECK(listen(server.listener, 64) == 0);
    CHECK(getsockname(server.listener, (struct sockaddr *)&address,
                      &address_length) == 0);
    CHECK(snprintf(authority, sizeof(authority), "localhost:%u",
