...
static outcome_t parse(
    ...
    outcome.start_hash = UINT64_C(1469598103934665603);
    ...
    {
        maelys_http_slice_t method = maelys_http_parser_method(parser);
        maelys_http_slice_t target = maelys_http_parser_target(parser);
        outcome.start_hash = hash_bytes(outcome.start_hash,
            (const unsigned char *)method.data, method.length);
        outcome.start_hash = hash_bytes(outcome.start_hash,
            (const unsigned char *)target.data, target.length);
    }
...
int LLVMFuzzerTestOneInput(const unsigned char *data, size_t size) {
    ...
    if (whole.result != bytewise.result || whole.framing != bytewise.framing ||
        ...
        whole.start_hash != bytewise.start_hash) abort();
    return 0;
}
